...
Copy the `<style>` block **verbatim** from `docs/licensing.html` (lines ~17–95, the entire `<style>…</style>`). Do not rewrite the CSS — it must match exactly. The file:
...
Run: `grep -c "font-family:'JetBrains Mono'" docs/academic.html`
Expected: `1` (the @font-face from the copied style block is present — confirms CSS was pasted, not left as a comment).
...
- [ ] **Step 2: Add the academic line in the products section.** Find the opening of the products section in `docs/index.html`:

```html
  <section id="products">
```

Read the lines immediately following it to locate the section's intro paragraph (the first `<p>` after the `<h2>`/`sec-num`). Append this sentence to the end of that intro paragraph's text, before its closing `</p>`:

```html
 Free for non-commercial academic use &mdash; <a href="academic.html">see the academic licence</a>.
```

If the products section has no intro paragraph (only a heading then the grid), instead insert this standalone paragraph on the line directly before the `<div class="grid` of the products section:
...
Load `http://localhost:8765/academic.html`, fill the form with test values (role=Student, product=jgs-magic-sysmlv1-pro), submit. Confirm the on-page success status appears and that an email arrives at `support@jgsystemsconsulting.com` with subject "Academic licence request (labs site)". Then delete the test submission from the Formspree dashboard (form `mjgqrjpa`) to keep the inbox clean.
...
- **Spec coverage:** §00–§04 page sections → Tasks 1–4. Eligibility (4 audiences) → §02 grid + clarifier. Citation as courtesy (not enforced) → §03 wording "appreciate — but do not require". Formspree reuse with academic fields + distinct `_subject` → Task 4. Discoverability (nav, footer, licensing callout, homepage line) → Tasks 5–6. Verification (HTML, render, links, e2e form) → Task 7. All spec sections mapped.
